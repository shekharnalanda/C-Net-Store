...
class _ImageLibraryScreenState extends State<ImageLibraryScreen> {
  ...
  void initState() {
    ...
    assets = ApiClient().productImageLibrary();
  }

  void reload() => setState(() => assets = ApiClient().productImageLibrary(search: search.text.trim(), group: group));
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Column(children: [
      Padding(padding: const EdgeInsets.all(12), child: Column(children: [
        TextField(controller: search, textInputAction: TextInputAction.search, onSubmitted: (_) => reload(), decoration: InputDecoration(hintText: 'Search biryani, grocery, beauty…', prefixIcon: const Icon(Icons.search), suffixIcon: IconButton(onPressed: reload, icon: const Icon(Icons.arrow_forward)))),
        FutureBuilder<List<dynamic>>(future: groups, builder: (_, snapshot) => SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [
          ChoiceChip(label: const Text('All'), selected: group == null, onSelected: (_) { group = null; reload(); }),
          ...?snapshot.data?.map((item) => Padding(padding: const EdgeInsets.only(left: 7), child: ChoiceChip(label: Text(item.toString()), selected: group == item.toString(), onSelected: (_) { group = item.toString(); reload(); }))),
        ]))),
      ])),
